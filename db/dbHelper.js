...
DatabaseHelper.prototype.loginStudent = function loginStudent(studentID, regID, locationID, center, cb) {
    ...
    self.db.query("SELECT id FROM centers WHERE centers.description = \'" + center + "\';", function (err1, results) {
       if (err1) {
           ...
           centerID = results;
           self.db.query("INSERT INTO students VALUES(" + parseInt(studentID) + ", " + parseInt(regID) + ", convert_tz(current_timestamp(), '+00:00', '-07:00'), convert_tz(current_timestamp(), '+00:00', '-07:00'), " +
...
DatabaseHelper.prototype.loginTutor = function loginTutor(studentID, tutorID, center, cb) {
    ...
    self.db.query("SELECT id FROM centers WHERE centers.description = \'" + center + "\';", function (err1, results) {
        if (err1) {
            ...
            console.log(results);
            centerID = results;
            self.db.query("INSERT INTO tutors VALUES(" + parseInt(studentID) + ", " + parseInt(tutorID) + ", " + requestable + ", " +
